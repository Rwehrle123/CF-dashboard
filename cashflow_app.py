...
with tabs[4]:
    ...
    for ci, (fyr, fmn) in enumerate(focus_months):
        ...
        with cols[ci]:
            ...
            # Verdict: is the 4+13 forecast AP within the maximum headroom?
            ap_vs_max           = max_ap_headroom - fc_ap_total_monthly
            if ap_vs_max < -50000:
                st.error(
                    f"🚨 **4+13 forecast AP ({fmt(fc_ap_total_monthly)}) exceeds maximum headroom "
                    f"({fmt(max_ap_headroom)}) by {fmt(abs(ap_vs_max))}** — "
                    f"reduce AP in the 4+13 tab to stay within cash target"
                )
            elif ap_vs_max < 200000:
                st.warning(
                    f"⚠️ **4+13 forecast AP ({fmt(fc_ap_total_monthly)}) is close to maximum "
                    f"headroom ({fmt(max_ap_headroom)})** — {fmt(ap_vs_max)} spare capacity"
                )
            else:
                st.success(
                    f"✅ **4+13 forecast AP within headroom** — "
                    f"{fmt(ap_vs_max)} spare capacity vs maximum"
                )
            ...
            # Three-column table: LY / 4+13 forecast / Maximum headroom
            col_hdr = "Remaining" if is_part else "Full month"
            st.markdown(
                f"| | LY run rate | 4+13 forecast | Max headroom |\n"
                f"|---|---|---|---|\n"
                f"| AP total | {fmt(ly_ap_total * frac_rem)} | **{fmt(fc_ap_total_monthly)}** | **{fmt(max_ap_headroom)}** |\n"
                f"| Weekly avg | {fmt(ly_ap_per_wk)} | **{fmt(fc_ap_per_wk)}** | **{fmt(max_ap_per_wk)}** |\n"
                f"| AP COGS | {fmt(ly_apcogs * frac_rem)} | {fmt(fc_ap_cogs_monthly)} | — |\n"
                f"| AP OVH  | {fmt(ly_apovh  * frac_rem)} | {fmt(fc_ap_ovh_monthly)}  | — |"
            )
            if is_part and ap_mtd > 0:
                st.caption(
                    f"AP already paid this month: **{fmt(ap_mtd)}** · "
                    f"Remaining max headroom: **{fmt(max_ap_headroom)}** · "
                    f"4+13 still to pay: **{fmt(fc_ap_total_monthly)}**"
                )
            if adj_note_3mo:
                st.caption(f"Slider adjustments applied: {adj_note_3mo.strip(' ·')}")
