...
if use_actual_pos and manual_open_week is not None:
    matches = [
        ...
        if pd.Timestamp(wk).normalize() == manual_open_week.normalize()
    ]
...
with tabs[4]:
    ...
    st.caption(
        ...
        f"This tab uses **total cash** and the same close chain as the Weekly 4+13 Outlook."
    )
    ...
    for ci, (fyr, fmn) in enumerate(focus_months):
        ...
        if is_part and use_actual_pos:
            ...
            open_cash = wk_open if wk_open is not None else book2_open_month
        ...
        with cols[ci]:
            ...
            st.markdown(
                ...
                "|---|---|---|\n"
                f"| Opening total cash | {fmt(book2_open_month)} | **{fmt(open_cash)}** |\n"
...
with tabs[6]:
    ...
    if use_actual_pos and pos_date is not None:
        anchor_week_for_msg = pd.Timestamp(pos_date) + pd.Timedelta(days=1)
        st.info(
            f"📍 Opening balance anchored to actual total cash position as at "
            f"**{pd.Timestamp(pos_date).strftime('%d %b %Y')}**: "
            f"SHT £{pos_sht:,.0f} + SHGI £{pos_shgi:,.0f} + TMD £{pos_tmd:,.0f} = **Total £{pos_total:,.0f}**. "
            f"This is used as the opening balance for "
            f"**W{anchor_week_for_msg.isocalendar()[1]} {anchor_week_for_msg.strftime('%d/%m')}** "
            f"= **£{open_base:,.0f}k**."
        )
...
- Opening balance date — must be a Sunday. This feeds the following Monday week.
...
""")
    ...
    with st.expander("📅 Manual opening balance logic", expanded=True):
        st.markdown("""
The sidebar opening balance date must be a **Sunday**.

That Sunday balance is treated as the closing position for that week and becomes the opening balance for the following Monday.
